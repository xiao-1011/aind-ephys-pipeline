#!/bin/bash
# ---------------------------------------------------------------------------
# Multi-shank — SLURM submission script for NAISS Dardel
# Runs KS4 + SpykingCircus2 + MountainSort5 + Tridesclous2 + Lupin
# with per-shank preprocessing and sorting.
#
# Usage:
...
#SBATCH --mem=8G
#SBATCH -J multishank-ephys-pipeline
#SBATCH -p shared
...
OUTPUT_BASE="/cfs/klemming/projects/supr/dmclab/ephys-pipeline-output/multishank"
...
echo "Pipeline:     ${PIPELINE_PATH}/projects/multishank/main.nf"
...
$NF_BIN \
    -C "${PIPELINE_PATH}/projects/multishank/nextflow.config" \
    -log "${LOGDIR}/${SLURM_JOB_ID}.nextflow.log" \
    run "${PIPELINE_PATH}/projects/multishank/main.nf" \
    -work-dir "${WORKDIR}" \
    -resume \
