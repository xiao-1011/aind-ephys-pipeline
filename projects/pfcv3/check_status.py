#!/usr/bin/env python3
"""
Pipeline status + failure analysis for pfcv3.

For each session/probe, checks which outputs exist on disk and,
...
Defaults point to the pfcv3-batch2 output on Dardel.
"""
...
DEFAULT_RESULTS = "/cfs/klemming/projects/supr/dmclab/ephys-pipeline-output/pfcv3-batch2/results"
...
# Analyzer outputs (dir name → short label)
ANALYZERS = {
    "analyzer_kilosort4":      "KS4-ana",
    "analyzer_spykingcircus2": "SC2-ana",
    "analyzer_tridesclous2":   "TDC2-ana",
    "analyzer_lupin":          "LPN-ana",
}

# Curation outputs (file name → short label)
CURATIONS = {
    ...
    "advanced_curation_lupin.json":          "LPN-cur",
}
...
OUTPUT_TO_TRACE = {
    ...
    "analyzer_kilosort4":    ("ANALYZE_KS4",      lambda s, p: f"{s}/{p}/sorter_kilosort4"),
    "analyzer_spykingcircus2": ("ANALYZE_SC2",    lambda s, p: f"{s}/{p}/sorter_spykingcircus2"),
    "analyzer_tridesclous2": ("ANALYZE_TDC2",     lambda s, p: f"{s}/{p}/sorter_tridesclous2"),
    "analyzer_lupin":        ("ANALYZE_LUPIN",    lambda s, p: f"{s}/{p}/sorter_lupin"),
    "advanced_curation_kilosort4.json":      ("ADVANCED_CURATE", lambda s, p: f"{s}/{p}/analyzer_kilosort4"),
    "advanced_curation_spykingcircus2.json": ("ADVANCED_CURATE", lambda s, p: f"{s}/{p}/analyzer_spykingcircus2"),
    "advanced_curation_tridesclous2.json":   ("ADVANCED_CURATE", lambda s, p: f"{s}/{p}/analyzer_tridesclous2"),
    "advanced_curation_lupin.json":          ("ADV_CURATE_LPN",  lambda s, p: f"{s}/{p}/analyzer_lupin"),
}
...
def scan_results(results_dir: Path) -> dict:
    """Returns {(session, probe): set_of_present_outputs}"""
    data = {}
    for session_dir in sorted(results_dir.iterdir()):
        ...
        for probe_dir in sorted(session_dir.iterdir()):
            ...
            for name in list(SORTERS) + list(ANALYZERS):
                if (probe_dir / name).is_dir():
                    present.add(name)
            for name in CURATIONS:
                if (probe_dir / name).is_file():
                    present.add(name)
            data[(session, probe)] = present
    return data
...
def cell(present, label):
    return "✓" if label in present else "✗"
...
def main():
    parser = argparse.ArgumentParser(description="pfcv3 pipeline status checker")
    ...
    if not traces:
        print("  [warn] no trace files found — failure detail unavailable", file=sys.stderr)
    ...
    for (session, probe), present in disk.items():
        cells = sep.join(f"{cell(present, out):^{col_w}}" for out in all_outputs)
        ...
        for out in all_outputs:
            if out not in present:
                missing_items.append((session, probe, out))
    ...
    for session, probe, out in missing_items:
        ...
        for i, a in enumerate(task_attempts, 1):
            print(f"  [{i}] job {a['job_id']:>10}  {a['status']:<10}  exit={a['exit']:<5}  "
                  f"duration={a['duration']:<14}  realtime={a['realtime']:<14}  peak_rss={a['peak_rss']}")
        print(f"  → {v}")
