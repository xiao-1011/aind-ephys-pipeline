...
## 2026-07-27
## 2026-07-29
- [Claude] **Discovered SORT_KS4_BATCH wait-loop bug**: batch8 retry job 22261435 exited 0 but only 3 of 4 sortings actually completed (1060141_day1_imec? failed silently). Root cause: the shell `wait $pid` loop in main.nf only propagates the LAST wait's return code — if probe 4 succeeded, whole SLURM job reports success even if probe 3 failed. This defeats the `errorStrategy='retry'` we just added, since Nextflow never sees the failure. **Fixed** in main.nf: track `fail=0; wait $pid || fail=1; ...; exit $fail`. Mirrored to Dardel.
- [Claude] Batch8 retry job 22261404 COMPLETED clean for 1053836_day4 (KS4=2 TDC2=2 cons=2 delta=2) but left 1060141_day1 at KS4=1 (the silent-failure bug above).
- [Claude] **Fixed 5 probes locally with `06-compare.py --input-prefix sorting_clean_` + `08-consensus-delta.py`** (same playbook as 986168 fix on 2026-07-27):
    - 1033993_day1_imec1 (was cons=1/2, KS4 had 0 clean units → consensus 0/0, expected)
    - 986171_day3_imec0 (was cons=1/2 → now 2/2, 130 consensus units)
    - 986235_day2_imec0 (was cons=0/2 → now 2/2, 170 consensus units)
    - 986235_day2_imec1 (was cons=0/2 → now 2/2, 130 consensus units)
    - 986235_day3_imec1 (was cons=1/2 → now 2/2, 124 consensus units)
- [Claude] Submitted narrow retry (job 22276399) for 1060141_day1 with the wait-loop bug now fixed. Expected: 2h wall, both probes get KS4.
- [Claude] **Reliability fix for cuFFT_INTERNAL_ERROR** — after batch8 job 22238986 (SORT_KS4_BATCH `[1060141_day1, 1053836_day4]`) still failed at exit 1 despite the sleep-120 + nvidia-smi guard: (1) bumped `main.nf` SORT_KS4_BATCH `sleep 120 → 180`, (2) added `errorStrategy = { task.attempt <= 1 ? 'retry' : 'ignore' }; maxRetries = 1` in `nextflow.config` for SORT_KS4_BATCH so cuFFT-crash batches auto-retry on a fresh SLURM slot. If retry also fails, workflow ignores and finishes (same behavior as before, allowing the narrow-staging manual retry to close the gap). Current batch8 orchestrator (job 22238475) is unaffected — Nextflow loaded main.nf + config at startup. Fixes take effect on the next submission.
- [Claude] Batch8 job 22238120 died in 3s — post-maintenance module bump had removed `PDC/24.11`, `apptainer/1.4.0-cpeGNU-24.11`, `miniconda3/25.3.1-1-cpeGNU-24.11` from compute nodes. Now only 26.03 stack available. Verified new modules + conda env + CPU SIF via test job 22238444. Bumped `slurm_submit_batch8.sh` (Dardel + workstation): `PDC/24.11 → 26.03`, `apptainer 1.4.0-cpeGNU-24.11 → 1.5.1-cpeGNU-26.03`, `miniconda3 25.3.1-1-cpeGNU-24.11 → 26.3.2-2-cpeGNU-26.03`. **Resubmitted as job 22238475** — now RUNNING.
- [Claude] **Submitted batch8** as job 22238120 (reason: Priority). 15 sessions across 5 animals: 1053836 (4d), 1060141 (4d), 1060359 (4d), 1020228 (1d), 1033998 (2d). Wall time 3-12:00:00. All batch7 fixes carry over (sleep 120 + nvidia-smi warmup, PREPROCESS retry pattern).
- [Claude] **986168 fixed locally** — skipped the 5h raw upload. Ran `06-compare.py --input-prefix sorting_clean_` + `08-consensus-delta.py` on the workstation (kilosort conda env, spikeinterface 0.102.1) for the 2 missing imec1 probes (day1: 21 consensus units; day3: 52 consensus units). day4 remains at 1/2 probes since imec1 was never recorded. 986168 now as complete as the raw data allows.
- [Claude] Cancelled 986168 raw rsync + deleted 29 GB partial that had landed on Dardel.
